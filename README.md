...
The selector uses a scalable, fully desaturated three-panel interface that mirrors the tablet hardware. Compact F1–F5 and F6–F10 mapping cards flank a wider alphabetized preset list. Full-width arrow buttons above and below the list move one preset at a time and are sized for pen input. Labeled hardware-only Mode Switch spacers sit between F2/F3 and F7/F8. Both mapping panels update as the highlight moves, showing larger friendly action names, shortcuts, holds, sequences, and pen-drag gestures without activating the previewed preset. Brightness, shapes, borders, and text labels distinguish the current selection, active preset, favorites, and customized bindings without using color.
